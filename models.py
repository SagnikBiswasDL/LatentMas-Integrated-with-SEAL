...
class ModelWrapper:
    def __init__(self, model_name: str, device: torch.device, use_vllm: bool = False, args = None):
        ...
        if args and getattr(args, "use_seal", False):
            ...
            self.seal = SealSteering(
                vector_path=args.seal_vector_path,
                layer=args.seal_layer,
                coef=args.seal_coef,
                mode=args.seal_mode,
                device=seal_device,
            )
        ...
        if self.use_vllm:
            ...
            if self.seal is not None and use_second_hf:
                self.seal.register(self.HF_model)
    ...
    def _latent_model(self) -> torch.nn.Module:
        if hasattr(self, "HF_model"):
            return self.HF_model
        return self.model

    def _with_latent_seal(self):
        if self.seal is not None:
            self.seal.set_latent(True)

    def _without_seal(self):
        if self.seal is not None:
            self.seal.set_latent(False)
            self.seal.set_text_mask(None)
    ...
    def generate_latent_batch(
        ...
        *,
        ...
        self._with_latent_seal()
        outputs = latent_model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            past_key_values=past_key_values,
            use_cache=True,
